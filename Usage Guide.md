<div id="top"></div>


<br/>
<div align="center">
  <a href="https://github.com/OneBigUnit/K-Lazer">
    <img src="https://github.com/OneBigUnit/K-Lazer/blob/Source/Images/logo.png" alt="Logo" width="500" height="271">
  </a>
  
  <br />
  <br />

  <p align="center">
    A sleek, simple, and intuitive screen overlay application made to facilitate teaching and learning
    <br />
    <br />
    <a href="#content"><strong>Find Out More »</strong></a>
    <br />
    <br />
  </p>
</div>

<div id="content"></div>

# K-Lazer Usage Guide

### Controls
* Press `Esc` while the application is not running in the background to exit
* Left click the *'Change Mode'* button to switch the laser pointer functionality on / off
* Left click while the laser pointer is active to cast the laser pointer onto the screen
